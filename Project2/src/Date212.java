	 ...
	 // private instance variables. 
		...
		private boolean isValid(String date) {
			...
			if (m > 12 || m < 1) // to check if it is a valid month
				return false;
			if (d > 31 || d < 1) // to check if the date is valid 
				return false;
...
//		20171002 would be returned as 'October 2, 2017'.
			...
			public void setMonth(int m) {
				if (m > 12 || m < 1) //to check if it is a valid month and if it is invalid it will throw the exception
					throw new IllegalArgumentException("Invalid month " );
				month = m;
			}
			
			public void setDay(int d) {
				if (d > 31 || d < 1) //to check if it is a valid day and if it is invalid it will throw the exception
